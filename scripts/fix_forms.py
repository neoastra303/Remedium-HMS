...
forms_to_fix = {
    ...
    "care_monitoring/templates/care_monitoring/patientcare_form.html": (
        "Patient Care Record",
        "",
    ),
    ...
    "medical_records/templates/medical_records/patient_document_form.html": (
        "Upload Document",
...
for path, (title, enctype) in forms_to_fix.items():
    ...
    if "crispy_forms_tags" not in content:
        f.write_text(
            CRISPY_TEMPLATE.format(title=title, enctype=enctype), encoding="utf-8"
        )
        print(f"Fixed: {path}")
    else:
