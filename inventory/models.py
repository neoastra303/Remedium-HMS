...
class InventoryItem(RemediumBaseModel):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="Name of the inventory item"
    )
    ...
    quantity = models.PositiveIntegerField(
        validators=[MinValueValidator(0)], help_text="Current quantity in stock"
    )
    unit = models.CharField(
        max_length=10, choices=UNIT_CHOICES, help_text="Unit of measurement"
    )
    ...
    supplier = models.CharField(
        max_length=100, blank=True, null=True, help_text="Supplier name"
    )
