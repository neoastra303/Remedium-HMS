...
class IntegrationListView(LoginRequiredMixin, PermissionRequiredMixin, generic.ListView):
    ...
    ALLOWED_ORDER_BY = ['system_name', 'system_type', 'status', 'last_sync',
                        '-system_name', '-system_type', '-status', '-last_sync']
...
class IntegrationDetailView(LoginRequiredMixin, PermissionRequiredMixin, generic.DetailView):
    model = ExternalIntegration
...
class IntegrationCreateView(LoginRequiredMixin, PermissionRequiredMixin, generic.CreateView):
    model = ExternalIntegration
...
class IntegrationUpdateView(LoginRequiredMixin, PermissionRequiredMixin, generic.UpdateView):
    model = ExternalIntegration
    fields = ['system_name', 'system_type', 'api_endpoint', 'notes']
    template_name = 'integration/integration_form.html'
    success_url = reverse_lazy('integration_list')
...
class IntegrationDeleteView(LoginRequiredMixin, PermissionRequiredMixin, generic.DeleteView):
    model = ExternalIntegration
