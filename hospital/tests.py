...
def make_admin():
    return User.objects.create_superuser(username='hosp_admin', password='pass', email='h@h.com')
...
class TestRoomAPI:
    ...
    def test_create_room(self, admin_client):
        ...
        r = admin_client.post('/api/v1/rooms/', {
            'ward': ward.pk, 'room_number': '201', 'capacity': 2
        })
        assert r.status_code == status.HTTP_201_CREATED
