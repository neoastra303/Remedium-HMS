    ...
    if request.user.is_authenticated:
        ...
        if staff_profile and staff_profile.role == 'DOCTOR':
            context['my_appointments'] = Appointment.objects.filter(
                doctor=staff_profile, appointment_date__date=today, status='Scheduled'
        ...
        if request.user.is_superuser or (staff_profile and staff_profile.role == 'ADMIN'):
            ...
            dept_stats = list(
                Staff.objects.values('department').annotate(count=Count('id')).order_by('-count')[:5]
            )
            if dept_stats:
...
@login_required
@user_passes_test(lambda u: u.is_superuser or (hasattr(u, 'staff_profile') and u.staff_profile.role == 'ADMIN'))
def audit_log(request):
    ...
    all_history = sorted(
        list(Patient.history.all()[:20]) + list(Staff.history.all()[:20]) +
